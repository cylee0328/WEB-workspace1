...
//		System.out.println("잘 실행 되나??");
		 ...
		 * 전송방식, 요청한 사용자의 ip등등
		 * 
		 * 두번째 매개변수인 HttpServletResponse response 에는 요청 처리 후 응답할 때 사용하는 객체
		 * 
		 * 요청을 처리하기 위해 요청시 전달된 값(사용자가 입력한 값)들을 뽑느다.
		...
		int age = Integer.parseInt(request.getParameter("age")); // 반환값이 무조건 문자열이기 떄문에 형변환 해줘야함.
		...
		// 일반적인 호출 : Servlet -> service(뽑아낸 값들을 넘기면서 비지니스로직 실행) -> Dao호출 - DB SQL문 실행 -결과값 반환
		
		// 위의 과정을 다 처리한 후 사용자가 보게될 응답페이지를 만들어서 전달해주기
		
		/*
		 * 장점 : Java코드 내에 작성하기 때문에 반복문, 조건문, 유용한 메소드를 사용할 수 있다.
		 * 단점 : 복잡, 혹시라도 나중에 html수정해야할 때 Java코드내에서 수정이 이루어지기 때문에
		 * 		 수정된 내용을 다시 반영시키고자 한다면 서버를 재실행해야한다. 사소한 오타로 에러날 수 있다.
		 */
			
		//* response 객체를 통해 사용자에게 html(응답화면)을 전달
		// 1) 이제부터 내가 출력할 내용은 문서형태이고 html이다. 더불어서 문자셋은 UTF-8이라는 것을 지
